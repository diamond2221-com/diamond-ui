...
<template>
  <div class="page-container page-theme">
    ...
    <dm-dialog :visible.sync="copyDialogVisible">
      <dm-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <dm-form-item label="<%= 3 >" prop="name">
          <dm-input v-model="copyForm.name"></dm-input>
        </dm-form-item>
      </dm-form>
      <div slot="footer" class="dialog-footer">
        <dm-button @click="closeCopyForm">{{getActionDisplayName('cancel')}}</dm-button>
        <dm-button type="primary" @click="copyToUser">{{getActionDisplayName('confirm')}}</dm-button>
      </div>
    </dm-dialog>
  </div>
</template>
