...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.className).to.equal('dm-select');
    expect(vm.$el.querySelector('.dm-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.dm-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.dm-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.dm-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" @change="handleChange">
            <dm-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.dm-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" @visible-change="handleVisibleChange">
            <dm-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.dm-input__icon.dm-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" value-key="id">
            <dm-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.dm-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.dm-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <dm-select v-model="value">
            ...
            <i slot="prefix" class="dm-input__icon dm-icon-search"></i>
          </dm-select>
        </div>
      `,
    ...
    expect(vm.$el.querySelector('.dm-input__icon').classList.contains('dm-icon-search')).to.be.true;
  });

  it('custom dm-option template', () => {
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value">
    ...
    expect(vm.$el.querySelector('.dm-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.dm-select-group__wrap');
    const options = groups[1].querySelectorAll('.dm-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.dm-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.dm-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <dm-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <dm-select ref="select"></dm-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" ref="select">
            <dm-option label="1" :value="1" />
          </dm-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.dm-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <dm-select v-model="value" @change="change" ref="select">
            <dm-option label="1" :value="1" />
            <dm-option label="2" :value="2" />
            <dm-option label="3" :value="3" />
          </dm-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <dm-select ref="select" v-model="value" filterable>
            <dm-option label="test" value="test" />
          </dm-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.dm-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.dm-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.dm-input__inner').value).to.equal('黄金糕');
  });
