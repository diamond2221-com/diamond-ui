...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.dm-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.dm-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'dm-icon-check',
      inactiveIconClass: 'dm-icon-close'
    });

    const icon = vm.$el.querySelector('.dm-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <dm-switch disabled v-model="value"></dm-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.dm-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('expand switch value', done => {
    vm = createVue({
      template: `
        <div>
          <dm-switch v-model="value" :active-value="onValue" :inactive-value="offValue"></dm-switch>
        </div>
      `,
      data() {
        return {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <dm-switch :value="true"></dm-switch>
        </div>
      `
    }, true);
  ...
  it('sets checkbox value', done => {
    vm = createVue({
      template: `
        <div>
          <dm-switch v-model="value"></dm-switch>
        </div>
      `,
      data() {
        return {
