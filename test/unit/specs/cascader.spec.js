...
const getMenus = el => el.querySelectorAll('.dm-cascader-menu');
const getOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.dm-cascader-node');
const selectedValue = ['zhejiang', 'hangzhou', 'xihu'];
const getCloseButton = el => el.querySelectorAll('i.dm-tag__close');
...
describe('Cascader', () => {
  ...
  it('expand and check', async() => {
    ...
    expect(checkedOption.querySelector('i.dm-icon-check')).to.exist;
  ...
  it('with default value', async() => {
    ...
    expect(getOptions(el, 2)[0].querySelector('i').className).to.includes('dm-icon-check');
    expect(vm.$el.querySelector('input').value).to.equal('Zhejiang / Hangzhou / West Lake');
  });
  ...
  it('clearable', async() => {
    ...
    await waitImmediate();
    const closeBtn = vm.$el.querySelector('i.dm-input__icon');
    ...
    vm = createVue({
      template: `
        <dm-cascader
          v-model="value"
          :options="options"
          :show-all-levels="false"></dm-cascader>
      `,
      data() {
        return {
          value: selectedValue,
  ...
  it('multiple mode', async() => {
    ...
    getOptions(document.body, 0)[0].querySelector('.dm-checkbox input').click();
    await waitImmediate();
    expect(vm.value.length).to.equal(3);
    expect(getCloseButton(vm.$el).length).to.equal(3);

    const tags = vm.$el.querySelectorAll('.dm-tag');
    const closeBtn = tags[0].querySelector('.dm-tag__close');
    expect(tags.length).to.equal(3);
    expect(closeBtn).to.exist;
    closeBtn.click();
    await waitImmediate();
    expect(vm.value.length).to.equal(2);
    expect(vm.$el.querySelectorAll('.dm-tag').length).to.equal(2);
  ...
  it('clearable in multiple mode', async() => {
    ...
    await waitImmediate();
    expect(getOptions(document.body, 0)[0].querySelector('.dm-checkbox.is-checked')).to.exist;
    triggerEvent(vm.$el, 'mouseenter');
    await waitImmediate();
    const closeBtn = vm.$el.querySelector('i.dm-input__icon');
    expect(closeBtn).to.exist;
  ...
  it('collapse tags', async() => {
    ...
    expect(tags[0].querySelector('.dm-tag__close')).to.exist;
    expect(tags[1].querySelector('.dm-tag__close')).to.be.null;
    tags[0].querySelector('.dm-tag__close').click();
    expect(tags[1].textContent).to.equal('+ 3');
    await waitImmediate();
    expect(vm.value.length).to.equal(3);
    vm.$el.querySelector('.dm-tag .dm-tag__close').click();
    await waitImmediate();
    vm.$el.querySelector('.dm-tag .dm-tag__close').click();
    await waitImmediate();
    expect(vm.$el.querySelector('.dm-tag')).to.exist;
    // disabled tag can not be closed
    expect(vm.$el.querySelector('.dm-tag .dm-tag__close')).to.be.null;
  });
  ...
  it('filterable', async() => {
    ...
    await wait(300);
    expect(body.querySelector('.dm-cascader__suggestion-list')).to.exist;
    expect(body.querySelectorAll('.dm-cascader__suggestion-item').length).to.equal(3);
    body.querySelectorAll('.dm-cascader__suggestion-item')[0].click();
    await waitImmediate();
    expect(vm.value).to.deep.equal(selectedValue);
  });
  ...
  it('filter method', async() => {
    ...
    triggerEvent(input, 'input');
    await wait(300);
    expect(body.querySelector('.dm-cascader__suggestion-item').textContent).to.equal('Zhejiang / Hangzhou / West Lake');
  });
});
