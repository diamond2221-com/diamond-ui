...
describe('Avatar', () => {
  ...
  it('size is string', () => {
    vm = createVue({
      template: `
        <dm-avatar size="small">
          user
        </dm-avatar>
      `
    }, true);
    const avatarElm = vm.$el;
    expect(avatarElm.classList.contains('dm-avatar--small')).to.be.true;
  });

  it('shape', () => {
    vm = createVue({
      template: `
        <dm-avatar size="small" shape="square">
          user
        </dm-avatar>
      `
    }, true);
    const avatarElm = vm.$el;
    expect(avatarElm.classList.contains('dm-avatar--square')).to.be.true;
  });

  it('icon avatar', () => {
    vm = createVue({
      template: `
        <dm-avatar icon="dm-icon-user-solid">
        </dm-avatar>
      `
    }, true);
    const avatarElm = vm.$el;
    const iconELm = avatarElm.children[0];
    expect(avatarElm.classList.contains('dm-avatar--icon')).to.be.true;
    expect(iconELm.classList.contains('dm-icon-user-solid')).to.be.true;
  });

  it('image avatar', () => {
    vm = createVue({
      template: `
        <dm-avatar src="${IMAGE_SUCCESS}"></dm-avatar>
      `
    }, true);
  ...
  it('image fallback', async() => {
    vm = createVue({
      template: `
        <dm-avatar src="${IMAGE_FAIL}" @error="errorHandler">
          fallback
        </dm-avatar>
      `,
      methods: {
  ...
  it('image fit', async() => {
    vm = createVue({
      template: `
        <div>
          <dm-avatar :src="url"></dm-avatar>
          <dm-avatar :src="url" v-for="fit in fits" :fit="fit" :key="fit"></dm-avatar>
        </div>
