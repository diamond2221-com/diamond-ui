...
describe('Image', () => {
  ...
  it('create', async() => {
    ...
    const placeholder = vm.$el.querySelector('.dm-image__placeholder');
    const error = vm.$el.querySelector('.dm-image__error');
    let img = vm.$el.querySelector('.dm-image__inner');
    expect(placeholder).to.exist;
    expect(error).to.be.null;
    expect(img).to.be.null;

    await wait();
    img = vm.$el.querySelector('.dm-image__inner');
  ...
  it('$attrs', async() => {
    ...
    await wait();
    const $img = vm.$el.querySelector('.dm-image__inner');
  ...
  it('pass event listeners', async() => {
    ...
    vm = createVue({
      template: `
        <dm-image @click="handleClick" :src="src"></dm-image>
      `,
      data() {
        return {
          src
    ...
    await wait();
    vm.$el.querySelector('.dm-image__inner').click();
    await wait();
  ...
  it('big image preview', async() => {
    vm = createVue({
      template: `
        <dm-image :src="src" :preview-src-list="srcList"></dm-image>
      `,
      data() {
        return {
          src: src,
    ...
    expect(vm.$el.querySelector('.dm-image-viewer__wrapper')).to.exist;
    vm.$el.querySelector('.dm-image-viewer__close').click();
    await wait(1000);
    expect(vm.$el.querySelector('.dm-image-viewer__wrapper')).to.not.exist;
  });
});
