...
describe('Transfer', () => {
  ...
  const createTransfer = (props, opts) => {
    return createVue(Object.assign({
      template: `
        <dm-transfer :data="testData" ref="transfer" ${props}>
        </dm-transfer>
      `,
  ...
  it('filterable', done => {
    ...
    const leftList = transfer.$el.querySelector('.dm-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout(_ => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector('.dm-transfer-panel__header .dm-checkbox__label');
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(transfer.querySelector('.dm-transfer-panel__list .dm-checkbox__label span').innerText).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList = vm.$refs.transfer.$el.querySelector('.dm-transfer-panel').__vue__;
  ...
  describe('target order', () => {
    it('original(default)', done => {
      ...
      setTimeout(() => {
        transfer.addToRight();
        setTimeout(() => {
          const targetItems = [].slice.call(vm.$el.querySelectorAll('.dm-transfer__buttons + .dm-transfer-panel .dm-transfer-panel__body .dm-checkbox__label span'));
