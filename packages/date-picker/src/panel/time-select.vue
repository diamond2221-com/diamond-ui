<template>
  <transition name="dm-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
      ref="popper"
      v-show="visible"
      :style="{ width: width + 'px' }"
      :class="popperClass"
      class="dm-picker-panel time-select dm-popper">
      <dm-scrollbar noresize wrap-class="dm-picker-panel__content">
  ...
  export default {
    ...
    methods: {
      ...
      scrollToOption(selector = '.selected') {
        const menu = this.$refs.popper.querySelector('.dm-picker-panel__content');
